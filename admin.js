...
createUserForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        await setDoc(doc(db, 'users', userCredential.user.uid), {
            ...
            role: 'user',
            adminId: currentUser.uid, // Save the admin's ID who created this user
            hashedPassword: userPassword.value, // Store password for future reference
...
addSingleContact.addEventListener('click', async () => {
    ...
    try {
        await addDoc(collection(db, 'contacts'), {
            ...
            adminId: currentUser.uid, // Add the adminId to the contact
            status: 'notCalled',
            createdAt: serverTimestamp()
        });
    ...
    try {
        ...
        validContacts.forEach(contact => {
            ...
            batch.set(newDocRef, {
                ...
                assignedTo: selectUser.value,
                workType: workType.value,
                adminId: currentUser.uid, // Add the adminId to each contact
                status: 'notCalled',
                createdAt: serverTimestamp()
            });
        });
...
async function setupProgressListener() {
    ...
    // Create base query - modified to include adminId filter
    let baseQuery = query(
        collection(db, 'contacts'),
        where('assignedTo', '==', progressUser.value),
        where('workType', '==', progressWorkType.value),
        where('adminId', '==', currentUser.uid) // Only show contacts created by this admin
    );
...
// Update loadUsers function to only show users created by the current admin
async function loadUsers() {
    try {
        // Modified query to only show users created by the current admin
        const q = query(
            collection(db, 'users'), 
            where('role', '==', 'user'),
...
// Update loadExistingUsers function to only show users created by current admin
                    ...
                    <table class="users-table">
                        ...
                        <tbody>
                            ${snapshot.docs.length === 0 ? 
                              '<tr><td colspan="5" style="text-align:center; padding: 20px;">No users found. Create a new user to get started.</td></tr>' : 
                              snapshot.docs.map(doc => {
                                const data = doc.data();
                                return `
...
// Update the setupProgressListener function to include adminId filter
async function setupProgressListener() {
    // Clear existing listeners
    if (progressUnsubscribe) {
        progressUnsubscribe();
